...
//  }
                                             ...
                                             double fractionFrom0)
  {
    ...
    double[] p = GeometryUtils.computePointOnLine (p0, p1, fractionFrom0);
  ...
  {
    return GeometryUtils.length ((double) x0, (double) y0,
                            (double) x1, (double) y1);
  }
  ...
  public static double length (int[] x, int[] y, boolean isClosed)
  {
    ...
    if (isClosed && nPoints > 1)
      length += GeometryUtils.length (x[nPoints-1], y[nPoints-1], x[0], y[0]);
                                                     ...
                                                     int x1, int y1)
  {
    ...
    if (GeometryUtils.isPointInsideRectangle (x0, y0, x1, y1, lx0, ly0) ||
        GeometryUtils.isPointInsideRectangle (x0, y0, x1, y1, lx1, ly1))
      return true;

    // If it intersects it goes through. Need to check three sides only.

    // Check against top rectangle line
    if (GeometryUtils.isLineIntersectingLine (lx0, ly0, lx1, ly1,
                                         x0, y0, x1, y0))
      return true;
                                                         ...
                                                         int x1, int y1)
  {
    ...
    if (GeometryUtils.isPointInsideRectangle (x[0], y[0], x0, y0, x1, y1))
      return true;
    ...
    for (int i = 1; i < x.length; i++) {
      if (x[i-1] != x[i] || y[i-1] != y[i])
        if (GeometryUtils.isLineIntersectingRectangle (x[i-1], y[i-1],
                                                  x[i], y[i],
                                                  x0, y0, x1, y1))
          return true;
    }
                                                        ...
                                                        int x1, int y1)
  {
    ...
    if (n == 1)
      return GeometryUtils.isPointInsideRectangle (x0, y0, x1, y1, x[0], y[0]);

    //
    // If the polyline constituting the polygon intersects the rectangle
    // the polygon does too.
    //
    if (GeometryUtils.isPolylineIntersectingRectangle (x, y, x0, y0, x1, y1))
      return true;

    // Check last leg as well
    if (GeometryUtils.isLineIntersectingRectangle (x[n-2], y[n-2], x[n-1], y[n-1],
                                              x0, y0, x1, y1))
      return true;
    ...
    if (GeometryUtils.isPointInsidePolygon (x, y, x0, y0) ||
        GeometryUtils.isPointInsideRectangle (x0, y0, x1, y1, x[0], y[0]))
      return true;
                                                 ...
                                                 double[] intersection)
  {
    ...
    double a0 = GeometryUtils.equals (x0, x1, LIMIT) ?
                INFINITY : (y0 - y1) / (x0 - x1);
    double a1 = GeometryUtils.equals (x2, x3, LIMIT) ?
                                                       ...
                                                       double x1, double y1)
  {
    ...
    for (int i = 0; i < nPoints; i++) {
      ...
      if (GeometryUtils.equals (x2, x3) && GeometryUtils.equals (y2, y3))
        continue;

      int type = GeometryUtils.findLineSegmentIntersection (x0, y0, x1, y1,
                                                       x2, y2, x3, y3,
                                                       intersection);
      ...
      if (type == -2) { // Overlapping
        ...
        int side = GeometryUtils.sameSide (x0, y0, x1, y1,
                                      x[p1], y[p1], x[p2], y[p2]);
  ...
  public static int[] createEllipse (int[] x, int[] y)
  {
    ...
    double dy = GeometryUtils.length (x0, y0, x[0], y[0]) * Math.sin (axisAngle);
                                             ...
                                             int[] position)
  {
    ...
    for (int i = 1; i < x.length; i++) {
      double legLength = GeometryUtils.length (x[i-1], y[i-1], x[i], y[i]);
    ...
    public static List<Double> circleLineIntersection(double lineConst, double circleCenter1, double circleCenter2, double circleRadius) {
        double c1 = Math.pow((lineConst-circleCenter1),2);
        double c2 = c1-Math.pow(circleRadius,2);
        double c3 = Math.pow(circleCenter2,2)+c2;
        double c4 = 2*circleCenter2;
        double d = Math.pow(c4,2)-4*c3;
        if(d<0) {
            return new ArrayList<Double>();
        }
        else if(d==0.0) {
            double sol = c4/2;
            List<Double> sols = new ArrayList<Double>();
            sols.add(sol);
            return sols;
        }
        else {
    ...
    public static List<Double> circleIntersectionWithVerticalLine(double xLine, double xCircleCenter, double yCircleCenter, double circleRadius) {
        return circleLineIntersection(xLine, xCircleCenter, yCircleCenter, circleRadius);
    }

    public static List<Double> circleIntersectionWithHorizontalLine(double yLine, double xCircleCenter, double yCircleCenter, double circleRadius) {
        return circleLineIntersection(yLine, yCircleCenter, xCircleCenter, circleRadius);
    }

    /**
     * Only intersection with segments that are oriented along x or y axis.
     * @param x1
     * @param y1
     * @param x2
     * @param y2
     ...
     * @return
     */
    public static List<Double> circleIntersectionWithLineSegment(double x1, double y1, double x2, double y2, double xCirc, double yCirc, double rCirc) {
        assert(x1==x2 || y1==y2);
        List<Double> retVal = new ArrayList<Double>();
        // vertical
        if(x1==x2) {
            List<Double> intersec = circleIntersectionWithVerticalLine(x1, xCirc, yCirc, rCirc);
            if(!intersec.isEmpty()) {
                for (Double double1 : intersec) {
                    if(pointInSegment(x1,double1,x1,y1,x2,y2)) {
                        retVal.add(double1);
                    }
                }
            }
        ...
        // horizontal
            ...
            List<Double> intersec = circleIntersectionWithHorizontalLine(y1, xCirc, yCirc, rCirc);
            if(!intersec.isEmpty()) {
                for (Double double1 : intersec) {
                    if(pointInSegment(double1,y1,x1,y1,x2,y2)) {
                        retVal.add(double1);
                    }
                }
            }
    ...
    /**
     * Only for segments that are oriented along x or y axis.
     * @param xp
     * @param yp
     * @param x1
     * @param y1
     ...
     * @return
     */
    public static boolean pointInSegment(double xp, double yp, double x1, double y1, double x2, double y2) {
        assert(x1==x2 || y1==y2);
        if(x1==x2) {
            return (xp==x1) && ((y1<=yp && yp<=y2) || (y2<=yp && yp<=y1));
        }
        else {
            return (yp==y1) && ((x1<=xp && xp<=x2) || (x2<=xp && xp<=x1));
        }
    }

    public static boolean pointInRectangle(double xp, double yp, double x1, double y1, double x2, double y2) {
        return (((x1<=xp && xp<=x2) || (x2<=xp && xp<=x1)) && ((y1<=yp && yp<=y2) || (y2<=yp && yp<=y1)));
    }

    public static List<double[]> allIntersectionsOfRectangleAndCircle(double x1, double y1, double x2, double y2, double xCirc, double yCirc, double rCirc) {
        List<double[]> retVal = new ArrayList<double[]>();
        List<Double> ldrd = circleIntersectionWithLineSegment(x1, y1, x2, y1, xCirc, yCirc, rCirc);
        retVal.addAll(fromPointList(ldrd,y1));
        List<Double> rdru = circleIntersectionWithLineSegment(x2, y1, x2, y2, xCirc, yCirc, rCirc);
        retVal.addAll(fromPointList(x2,rdru));
        List<Double> rulu = circleIntersectionWithLineSegment(x2, y2, x1, y2, xCirc, yCirc, rCirc);
        retVal.addAll(fromPointList(rulu,y2));
        List<Double> luld = circleIntersectionWithLineSegment(x1, y2, x1, y1, xCirc, yCirc, rCirc);
        retVal.addAll(fromPointList(x1,luld));
        return retVal;
    }

    private static List<double[]> fromPointList(List<Double> points, double point) {
        List<double[]> retVal = new ArrayList<double[]>();
        for(Double d : points)  {
            retVal.add(new double[] {d,point});
        }
        return retVal;
    }

    private static List<double[]> fromPointList(double point, List<Double> points) {
        List<double[]> retVal = new ArrayList<double[]>();
        for(Double d : points)  {
            retVal.add(new double[] {point,d});
        }
        return retVal;
    }
    ...
    public static class Arc {
        ...
        public Arc(double[] firstPoint, double[] secondPoint) {
            this.startAngle = angleFromCoords(firstPoint);
            double auxEndAngle = angleFromCoords(secondPoint);
            if(startAngle>auxEndAngle) {
                auxEndAngle+=2*Math.PI;
            }
            endAngle = auxEndAngle;
        }
        ...
        @Override
        public int hashCode() {
            return new Double(startAngle).hashCode() + new Double(endAngle).hashCode();
        }
        ...
        public boolean equals(Object obj) {
            if(obj instanceof Arc) {
                Arc a = (Arc) obj;
                return (a.startAngle==startAngle && a.endAngle==endAngle);
            }
            return false;
        }
    }

    public static List<Arc> getArcsOfCircleInsideRectangle(double x1, double y1, double x2, double y2, double xCirc, double yCirc, double rCirc) {
        assert(pointInRectangle(xCirc, yCirc, x1, y1, x2, y2));
        List<Arc> retVal = new ArrayList<Arc>();
        if(isCircleInsideRectangle(x1, y1, x2, y2, xCirc, yCirc, rCirc)) {
            Arc a = new Arc(0.0, 2*Math.PI);
            retVal.add(a);
            return retVal;
        }
        List<double[]> allInters = allIntersectionsOfRectangleAndCircle(x1, y1, x2, y2, xCirc, yCirc, rCirc);
        for (double[] ds : allInters) {
            ds[0]-=xCirc;
            ds[1]-=yCirc;
        }
        Collections.sort(allInters,new AntiClockWiseComparator());
        int allIntersNum = allInters.size();
        for(int i = 0; i < allIntersNum; i++) {
            int nextI = (i+1)%allIntersNum;
            double[] firstPoint = allInters.get(i);
            double[] firstPointAux = Arrays.copyOf(firstPoint, 2);
            firstPointAux[0]+=xCirc;
            firstPointAux[1]+=yCirc;
            double[] secondPoint = allInters.get(nextI);
            double[] secondPointAux = Arrays.copyOf(secondPoint, 2);
            secondPointAux[0]+=xCirc;
            secondPointAux[1]+=yCirc;
            if(isArc(firstPointAux,secondPointAux,x1,y1,x2,y2,xCirc,yCirc,rCirc)) {
                Arc a = new Arc(firstPoint, secondPoint);
                retVal.add(a);
            }
        }
        return retVal;
    ...
    public static boolean isCircleInsideRectangle(double x1, double y1, double x2, double y2, double xCirc, double yCirc, double rCirc) {
        return (pointInRectangle(xCirc+rCirc, yCirc, x1, y1, x2, y2) &&
                pointInRectangle(xCirc-rCirc, yCirc, x1, y1, x2, y2) &&
                pointInRectangle(xCirc, yCirc+rCirc, x1, y1, x2, y2) &&
                pointInRectangle(xCirc, yCirc-rCirc, x1, y1, x2, y2));
    }

    /**
     * first point is assumed to be less anticlockwise than the second point
     * @param firstPoint
     * @param secondPoint
     * @param x1
     * @param y1
     ...
     * @return
     */
    public static boolean isArc(double[] firstPoint, double[] secondPoint, double x1, double y1, double x2, double y2, double xCirc, double yCirc, double rCirc) {
        double angle1 = angleFromCoords(firstPoint[0]-xCirc,firstPoint[1]-yCirc);
        double angle2 = angleFromCoords(secondPoint[0]-xCirc,secondPoint[1]-yCirc);
        if(angle2<angle1) {
            angle2+=2*Math.PI;
        }
        double midAngle = (angle1+angle2)/2;
        double[] middlePoint = new double[] {xCirc+Math.cos(midAngle)*rCirc,yCirc+Math.sin(midAngle)*rCirc};
        return pointInRectangle(middlePoint[0], middlePoint[1], x1, y1, x2, y2);
    }

    public static class AntiClockWiseComparator implements Comparator<double[]> {

        @Override
        public int compare(double[] o1, double[] o2) {
            return (angleFromCoords(o1)<angleFromCoords(o2))?-1:1;
        }
    ...
    /**
     * @param p double[] {x, y}
     * @return interval &lt;0; 2*PI) (anticlockwise, 0 = {1, 0})
     */
    public static double angleFromCoords(double[] p) {
        return angleFromCoords(p[0],p[1]);
    }
    ...
    /**
     * Returns an angle of a vector = to - from
     *
     * @param from
     * @param to
     * @return interval &lt;0; 2*PI) (anticlockwise, 0 = {1, 0})
     */
    public static double angleFromTwoPoints(Point3d from, Point3d to) {
        assert(from.z==to.z);
        double x = to.x-from.x;
        double y = to.y-from.y;
        return angleFromCoords(x,y);
    }

    /**
     *
     * @param fx from vector x
     * @param fy from vector y
     * @param tx to vector x
     * @param ty to vector y
     * @return intervals &lt;0; PI>, (-PI; 0&gt;  (anticlockwise, 0 = {1, 0})
     */
    public static double angleBetweenVectors(double fx, double fy, double tx, double ty) {
        double angle = Math.atan2(ty, tx) - Math.atan2(fy, fx);
        if (angle > Math.PI) {
            angle -= 2.0 * Math.PI;
        }
        if (angle < -Math.PI) {
     ...
     * @return intervals &lt;0; PI>, (-PI; 0&gt;  (anticlockwise, 0 = {1, 0})
     */
    public static double angleFromThreePoints(Point3d apex, Point3d from, Point3d to) {
        return angleBetweenVectors(from.x - apex.x, from.y - apex.y, to.x - apex.x, to.x - apex.x);
    }
